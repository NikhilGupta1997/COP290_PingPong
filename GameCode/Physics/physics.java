...
public class physics {
	...
	public int collision_wall(double posx,double posy,double radius,double x) {   //x is the dimension of the square
		int wall_no=0;
		if((posx+radius)>=x-20&&(posy<530&&posy>50)) 
			wall_no=4;
		else if((posy+radius+24.0)>=x-20&&(posx<530&&posx>50)) 
			wall_no=3;
		else if((posx-radius)<=20.0&&(posy<530&&posy>50)) 
			wall_no=2;
		else if((posy-radius+12.5-20)<=0.0&&(posx<530&&posx>50)) 
			wall_no=1;
		return wall_no;
	}
	...
	public int collision_corner(double posx,double posy,double radius,double x,double l ) { //y is the dimension of the corner
		...
		if((Math.abs(posx+posy-l)/sqrt2)<=radius) 
			corner_no=1;//working well
		else if(Math.abs(posy+posx-2*x+l)/sqrt2<=radius+20.0)
			corner_no=3;//working well
		else if(Math.abs(posy-posx+x-l)/sqrt2<=radius) 
			corner_no=2;// working well
		else if(Math.abs(posy-posx-x+l)/sqrt2<=radius+22.0) 
			corner_no=4;
		return corner_no;
	}

	public boolean collision_paddle(double posx,double posy,double len,int X,double radius,double padd_x,double padd_y,double h,int paddle_no) {  // TODO: Synchronize the player no 
		if(paddle_no==1) {
			if(posx<=(padd_x+len/2) && posx>=(padd_x-len/2) && posy<=(h+radius-10.0)) 
				return true; 
			else 
				return false;
		}
		else if(paddle_no==4) {
			if(posy<=(padd_y+len/2) && posy>=(padd_y-len/2) && posx >= X-(h+radius+7.5)) 
				return true; 
			else 
				return false ;
		}
		else if(paddle_no==3) {
			if(posx<=(padd_x+len/2) && posx>=(padd_x-len/2) && posy >= X-(h+radius+22)) 
				return true; 
			else 
				return false;
		}
		else {
			if(posy<=(padd_y+len/2) && posy>=(padd_y-len/2) && posx<=(h+radius)) 
				return true; 
			else 
				return false ;
		}
